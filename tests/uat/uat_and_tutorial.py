...
def __(config):
    ...
    _marimo.md("## Zero-Shot Distillation (Scenarios 1 & 2) & Scheduler Parallelization")

    with _tempfile.TemporaryDirectory() as _dist_dir:
        dist_path = _Path(_dist_dir)
        ...
        def mock_pace_activeset(cmd, **kwargs):
            ...
            out_path = _PathLocal(cmd[out_idx + 1])
        ...
        with (
            _patch("pyacemaker.core.active_set.run_command", side_effect=mock_pace_activeset),
            _patch("shutil.which", return_value="/usr/bin/pace_activeset"),
        ):
        ...
        with (
            _patch2("pyacemaker.core.trainer.run_command", side_effect=mock_pace_train),
            _patch2("shutil.which", return_value="/usr/bin/pace_train"),
        ):
...
def __():
    ...
    mock_elastic.engine.relax.return_value = _Atoms("Fe")

    mock_report = _MagicMock()
    ...
    with _tempfile.TemporaryDirectory() as _vdir:
        ...
        result = validator.validate(
            _dummy_pot,
            report_path,
            structure=_Atoms("Fe", cell=[2.8, 2.8, 2.8], positions=[[0, 0, 0]], pbc=True),
        )
...
def __(config):
    ...
    _marimo.md("## The Active Learning Event: Two-Tier Thresholds & Cutout (Scenario 3)")
    ...
    from pyacemaker.utils.extraction import (
        extract_intelligent_cluster as _extract_intelligent_cluster,
    )

    cluster = _extract_intelligent_cluster(halt_structure, target_atoms, config.cutout)
...
def __():
    ...
    _marimo.md("## Hierarchical Fine-Tuning & Master-Slave Resume (Scenario 4)")

    import tempfile as _tempfile2
    from pathlib import Path as _Path2

    with _tempfile2.TemporaryDirectory() as _temp_dir2:
        temp_path2 = _Path2(_temp_dir2)

        # 1. Finetune MACE
        dataset_path2 = temp_path2 / "dataset.xyz"
        from ase import Atoms as _Atoms4
        from ase.io import write as _write

        _write(dataset_path2, _Atoms4("Fe"), format="extxyz")
        ...
        from pyacemaker.core.trainer import PacemakerTrainer as _PacemakerTrainer2

        trainer2 = _PacemakerTrainer2(training_config)
        from pyacemaker.domain_models.workflow import LoopStrategyConfig as _LoopStrategyConfig

        _strategy_config = _LoopStrategyConfig()
        ...
        with (
            _patch3("pyacemaker.core.trainer.run_command", side_effect=mock_pace_train2),
            _patch3("shutil.which", return_value="/usr/bin/pace_train"),
        ):
        ...
        from unittest.mock import MagicMock as _MagicMock4

        mock_driver = _MagicMock4()
        ...
        from unittest.mock import patch as _patch4

        with _patch4("pyacemaker.core.engine.LammpsDriver", return_value=mock_driver):
            engine.run(
                _Atoms4("Fe", cell=[2.8, 2.8, 2.8], positions=[[0, 0, 0]], pbc=True),
                new_pot,
...
def __():
    ...
    _marimo.md("## State Resilience & Checkpointing (Scenario 5)")

    import tempfile as _tempfile3
    from pathlib import Path as _Path3

    with _tempfile3.TemporaryDirectory() as _temp_dir3:
        temp_path3 = _Path3(_temp_dir3)
