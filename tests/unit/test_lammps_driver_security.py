...
@pytest.fixture
def driver() -> LammpsDriver:
    with patch("pyacemaker.interfaces.lammps_driver.lammps"):
        return LammpsDriver()


def test_validate_command_safe(driver: LammpsDriver) -> None:
...
def test_validate_command_unsafe_chars(driver: LammpsDriver) -> None:
...
def test_validate_command_shell_token(driver: LammpsDriver) -> None:
