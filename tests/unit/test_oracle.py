...
def test_dft_manager_unhappy_path_io_error(mock_dft_config: DFTConfig) -> None:
    """Test compute handles unexpected IOError from engine."""
    atoms = Atoms("H", cell=[10, 10, 10], pbc=True)
    mock_driver = MagicMock()

    # Setup mock calculator to raise IOError
    class IOErrorCalculator(MockCalculator):
        def calculate(self, *args, **kwargs):
            raise OSError("Simulated IO Error (e.g. disk full)")

    mock_driver.get_calculator.return_value = IOErrorCalculator()
    manager = DFTManager(mock_dft_config, driver=mock_driver)

    gen = manager.compute(iter([atoms]))
    # It should catch it as an OracleError
    with pytest.raises(OracleError, match="Oracle calculation failed.*Simulated IO Error"):
        next(gen)


def test_dft_manager_unhappy_path_corrupted_data(mock_dft_config: DFTConfig) -> None:
    """Test compute handles corrupted inputs gracefully."""
    # Create invalid atom
    atoms = Atoms("H", cell=[0, 0, 0], pbc=True)
    # The actual ASE get_calculator might fail immediately if cell is invalid depending on driver.
    # We mock it to raise an exception.
    mock_driver = MagicMock()
    mock_driver.get_calculator.side_effect = ValueError("Corrupted structure data")

    manager = DFTManager(mock_dft_config, driver=mock_driver)
    gen = manager.compute(iter([atoms]))

    with pytest.raises(OracleError, match="Oracle calculation failed.*Corrupted structure data"):
        next(gen)
