...
def test_extract_local_region_basic(element: str, crystal_structure: str, a: float) -> None:
    ...
    atoms: Atoms = bulk(element, crystal_structure, a=a).repeat((3, 3, 3))  # type: ignore[no-untyped-call]
    ...
    cluster: Atoms = extract_local_region(atoms, center_idx, radius, buffer)
...
def test_extract_local_region_pbc() -> None:
    ...
    atoms: Atoms = bulk("Cu", "sc", a=2.5).repeat((2, 2, 2))  # type: ignore[no-untyped-call]
    # 8 atoms.
    # Center at 0 (corner).
...
def test_pre_relax_buffer() -> None:
    ...
    atoms: Atoms = Atoms("Cu3", positions=[[0, 0, 0], [2.5, 0, 0], [5.0, 0, 0]])

    # Core atoms (weight 1.0) should be fixed, buffer (weight 0.0) should move
    weights: np.ndarray = np.array([1.0, 0.0, 0.0])
    atoms.new_array("force_weight", weights)  # type: ignore[no-untyped-call]

    # Store original positions
    original_positions: np.ndarray = atoms.get_positions().copy()  # type: ignore[no-untyped-call]

    # Relax
    relaxed_atoms: Atoms = _pre_relax_buffer(atoms)
...
def test_passivate_surface() -> None:
    ...
    atoms: Atoms = Atoms("O3", positions=[[0, 0, 0], [2.0, 0, 0], [4.0, 0, 0]])

    # Only middle atom is core
    weights: np.ndarray = np.array([0.0, 1.0, 0.0])
    atoms.new_array("force_weight", weights)  # type: ignore[no-untyped-call]

    # Passivate
    passivated_atoms: Atoms = _passivate_surface(atoms, element="H")
    ...
    symbols: list[str] = passivated_atoms.get_chemical_symbols()  # type: ignore[no-untyped-call]
    assert "H" in symbols

    new_weights: np.ndarray = passivated_atoms.get_array("force_weight")  # type: ignore[no-untyped-call]
...
def test_extract_intelligent_cluster(element: str, crystal_structure: str, a: float) -> None:
    ...
    # Add dummy c_gamma
    c_gamma: np.ndarray = np.random.rand(len(atoms))
    ...
    empty_cluster: Atoms = extract_intelligent_cluster(atoms, [], config)
    assert len(empty_cluster) == len(atoms)

    # Target the center atom
    center_idx = 13
    cluster: Atoms = extract_intelligent_cluster(atoms, [center_idx], config)
