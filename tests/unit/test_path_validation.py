...
def test_validate_path_safe_outside_allowed() -> None:
    ...
    if Path("/etc/passwd").exists():
        # Error changed due to path.resolve(strict=True) raising on some OS/contexts before check_allowed_roots
        # But validate_path_safe catches it and re-raises as "Invalid path resolution" or "Path traversal detected"
