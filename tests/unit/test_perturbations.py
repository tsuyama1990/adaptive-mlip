...
def test_rattle() -> None:
    ...
    original_pos = getattr(atoms, "get_positions")()

    # Rattle
    new_atoms = rattle(atoms, stdev=0.1)

    # Check that positions changed
    assert not np.allclose(original_pos, getattr(new_atoms, "get_positions")())

    # Check that cell is same
    assert np.allclose(getattr(atoms, "get_cell")(), getattr(new_atoms, "get_cell")())

    # Check that original atoms not modified
    assert np.allclose(getattr(atoms, "get_positions")(), original_pos)
...
def test_apply_strain() -> None:
    atoms = bulk("Al", "fcc", a=4.0)
    original_cell = getattr(atoms, "get_cell")()
    ...
    assert np.allclose(getattr(new_atoms, "get_cell")(), expected_cell)
...
def test_create_vacancy() -> None:
    ...
    atoms = getattr(bulk("Al", "fcc", a=4.0), "repeat")((3, 3, 3))
