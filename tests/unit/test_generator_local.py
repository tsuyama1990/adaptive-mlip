...
def test_generate_local_rattle():
    ...
    # Fix Memory Safety Violation: Use streaming/islice instead of list(...) if large.
    # We just want to check a few.
...
def test_generate_local_md_burst():
    ...
    # Mock PolicyFactory to return our MagicMock so we don't depend on real MDMicroBurstPolicy
    with patch("pyacemaker.core.generator.PolicyFactory.get_local_policy") as mock_get_policy:
        mock_policy = MagicMock()
        # Mock policy.generate to return an iterator of mock structures
        mock_policy.generate.return_value = iter([Atoms("He")])
        mock_get_policy.return_value = mock_policy

        iterator = generator.generate_local(
            base, n_candidates=1, engine=real_mock_engine, potential="pot"
        )
        candidates = list(islice(iterator, 1))

        assert len(candidates) == 1
        assert candidates[0].get_chemical_symbols() == ["He"]
        # Verify policy was called with correct kwargs
        mock_policy.generate.assert_called_once_with(
            base, config, n_structures=1, engine=real_mock_engine, potential="pot"
        )
