...
def test_generator_watchdog(tmp_path: Path) -> None:
    ...
    assert "fix py_invoke all python/invoke 10 post_force invoke_evaluator" in script
    assert "fix halt_check all halt 10 v_exceed_flag != 0 error continue" in script
