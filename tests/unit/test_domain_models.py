...
def test_dft_config_invalid_encut() -> None:
    with pytest.raises(ValidationError):
        DFTConfig(code="qe", functional="PBE", kpoints_density=0.04, encut=-100.0)  # Must be positive
...
def test_training_config_invalid_cutoff() -> None:
    with pytest.raises(ValidationError):
        TrainingConfig(potential_type="ace", cutoff_radius=-2.0, max_basis_size=500)  # Must be positive
