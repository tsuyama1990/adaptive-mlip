...
def test_validate_lammps_command_invalid_chars() -> None:
    with pytest.raises(
        ValueError,
        match="contains explicitly blocked shell metacharacters|contains forbidden characters",
    ):
...
def test_validate_lammps_command_invalid_command() -> None:
    with pytest.raises(
        ValueError, match="forbidden or unrecognized command|contains forbidden characters"
    ):
