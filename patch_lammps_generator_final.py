...
for line in lines:
    if line.strip() == 'buffer.write(f"compute gamma all pace {safe_pot_path!s}\\n")':
        ...
        new_lines.append(indent + "has_active = hasattr(self.config, 'custom_initialization_commands') and self.config.custom_initialization_commands and any('active_atoms' in cmd for cmd in self.config.custom_initialization_commands)\n")
        new_lines.append(indent + "target_group = 'active_atoms' if has_active else 'all'\n")
