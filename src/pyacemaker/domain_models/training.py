...
class TrainingConfig(BaseModel):
    ...
    max_iterations: int = Field(
        default=DEFAULT_TRAINING_MAX_ITERATIONS, description="Maximum training iterations", gt=0
    )
    batch_size: int = Field(
        default=DEFAULT_TRAINING_BATCH_SIZE, description="Training batch size", gt=0
    )
    elements: list[str] | None = Field(
        default=None, description="List of chemical elements in the dataset (optional optimization)"
    )
    ...
    output_filename: str = Field(
        default=FILENAME_POTENTIAL, description="Filename for the trained potential"
    )
    ...
    # Spec Section 3.3
    delta_learning: bool = Field(default=False, description="Use LJ baseline for delta learning")
    active_set_optimization: bool = Field(
        default=False, description="Use MaxVol selection for active set"
    )
    active_set_size: int | None = Field(
        default=None, description="Target number of structures for active set", gt=0
    )
