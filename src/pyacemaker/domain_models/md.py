...
class MDConfig(BaseModel):
    ...
    velocity_seed: PositiveInt = Field(
        DEFAULT_MD_VELOCITY_SEED, description="Seed for random velocity generation"
    )
    neigh_modify_delay: int = Field(
        DEFAULT_MD_NEIGH_DELAY, ge=0, description="Neighbor list rebuild delay"
    )
    neigh_modify_every: int = Field(
        DEFAULT_MD_NEIGH_EVERY, ge=1, description="Neighbor list rebuild frequency"
    )
    neigh_modify_check: str = Field(
        DEFAULT_MD_NEIGH_CHECK, pattern="^(yes|no)$", description="Neighbor list rebuild check"
    )
