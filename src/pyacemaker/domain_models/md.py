...
class MDConfig(BaseModel):
    ...
    base_energy: float = Field(
        DEFAULT_MD_BASE_ENERGY, description="Baseline energy for initial base simulation defaults"
    )
    default_forces: list[list[float]] = Field(
        default=[[0.0, 0.0, 0.0]], description="Default forces for fallback simulation"
    )
