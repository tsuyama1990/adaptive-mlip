...
class LammpsEngine(BaseEngine):
    ...
    def _validate_script_content(self, script_path: Path) -> None:
        ...
        for line in script_content.splitlines():
            # Check if the line strictly begins with the shell command
    ...
    def _execute_simulation(self, driver: LammpsDriver, script_path: Path) -> None:
        ...
        try:
            ...
            # Driver raises RuntimeError if subprocess fails.
            # MDHaltInterrupt from evaluator will just pass through natively since python 3 allows custom exceptions to bubble up
            # if we explicitly call it. We don't parse strings.
    ...
    def run(
        ...
        with ctx:
            ...
            gen_ctx = ScriptGenerationContext(
            ...
            with input_script_path.open("w") as f:
                self.generator.write_script(f, gen_ctx)
            ...
            try:
                ...
                if self.config.evaluator_thresholds and max_gamma > self.config.evaluator_thresholds.threshold_call_dft:
                    halted = True
