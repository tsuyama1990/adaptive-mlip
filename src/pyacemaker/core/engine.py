            ...
            if self.thresholds.ignored_atoms:
                ...
                try:
                    ...
                    if gamma_array_ptr:
                        ...
                        gamma_data = np.ctypeslib.as_array(
                            ctypes.cast(gamma_array_ptr, ctypes.POINTER(ctypes.c_double)),
                            shape=(nlocal,),
                        )

                        # Get atom IDs to filter
                        atom_ids_ptr = lmp.extract_atom("id", 0)
                        atom_ids = np.ctypeslib.as_array(
                            ctypes.cast(atom_ids_ptr, ctypes.POINTER(ctypes.c_int)), shape=(nlocal,)
                        )
                        ...
                        max_gamma = float(np.max(gamma_data[mask])) if np.any(mask) else 0.0
                except Exception as e:
                    logger.warning(
                        f"Could not extract per-atom gamma to filter ignored atoms: {e}. Using global max."
                    )
        except Exception:
