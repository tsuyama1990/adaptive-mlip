        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if attempt == self.max_retries - 1:
                        msg = f"Failed to extract max_g variable after {self.max_retries} retries."
                        raise RuntimeError(msg) from err
                    time.sleep(self.base_backoff * (2 ** attempt)) # exponential backoff
            ...
            if self.consecutive_exceedances >= self.smooth_steps:
                ...
                try:
                    ...
                    if ptr:
                            # Validate pointer and cast safely
                            if not isinstance(ptr, int) and getattr(ptr, "value", None) is None:
                                msg = "Invalid pointer returned from LAMMPS extract_compute."
