                ...
                if result and result.trajectory_path:
                    ...
                    if traj_path.exists() and traj_path.is_file():
                        try:
                            ...
                            # Use iread to support potentially large trajectories efficiently
                            # Keep only the last frame generated by the microburst
                            last_frame = None
                            for frame in iread(traj_path, index=":"):
                                last_frame = frame
                            if isinstance(last_frame, Atoms):
                                yield last_frame
                                continue
                        except OSError as e:
    ...
    def generate(
        ...
        for _ in range(n_structures):
            ...
            if epicenter_indices:
                ...
                # Use a smooth radial mask based on distances from epicenters
                mask = np.zeros(len(positions))

                # Fetch positions to calculate distances
                for epi_idx in epicenter_indices:
                    epi_pos = positions[epi_idx]
                    # Calculate distances from the epicenter
                    dist = np.linalg.norm(positions - epi_pos, axis=1)
                    # Use a decaying Gaussian function for the mask
                    # Adjust the variance (e.g., 2.0) based on typical bond lengths to localize
                    decay = np.exp(-(dist**2) / (2.0 * 2.0**2))
                    # Use maximum to aggregate multiple epicenters
                    mask = np.maximum(mask, decay)

                # Boost the magnitude at epicenters
                displacement *= mask[:, np.newaxis] * 2.0
