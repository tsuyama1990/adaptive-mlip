...
from pyacemaker.core.executor import PacemakerExecutor
...
class PacemakerTrainer(BaseTrainer):
    ...
    Coherence class that coordinates configuration generation and execution.
    """

    def __init__(self, config: TrainingConfig) -> None:
        self.config = config
        self.config_generator = PacemakerConfigGenerator(config)
        self.executor = PacemakerExecutor()
    ...
    def train(self, training_data_path: str | Path) -> Any:
        ...
        # Execute training
        self.executor.run_training(input_yaml_path)
