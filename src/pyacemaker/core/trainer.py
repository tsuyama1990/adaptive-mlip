...
from pyacemaker.domain_models.training import MaceFinetuneConfig, TrainingConfig
    ...
    def _validate_training_data(self, data_path: Path) -> None:
        ...
        try:
            ...
            if not len(first_frame):
                self._raise_empty_frame(data_path)
        except Exception as e:
            if isinstance(e, TrainerError):
                raise
            self._raise_parse_error(e)

    def _raise_empty_frame(self, data_path: Path) -> None:
        msg = f"First frame of training data is empty: {data_path}"
        raise TrainerError(msg)

    def _raise_parse_error(self, e: Exception) -> None:
...
class FinetuneManager:
    ...
    def __init__(self, config: "MaceFinetuneConfig | None" = None) -> None:
        if config is None:
            from pyacemaker.domain_models.training import MaceFinetuneConfig
            config = MaceFinetuneConfig()
        self.config = config
    ...
    def finetune(self, dataset_path: str | Path) -> str:
        ...
        cmd = [
            ...
            "--valid_fraction", str(self.config.valid_fraction),
            "--E0s", self.config.E0s,
            "--model", self.config.model_architecture,
            "--num_interactions", str(self.config.num_interactions),
            "--max_num_epochs", str(self.config.max_num_epochs),
            "--start_swa", str(self.config.start_swa),
            "--scheduler_patience", str(self.config.scheduler_patience),
