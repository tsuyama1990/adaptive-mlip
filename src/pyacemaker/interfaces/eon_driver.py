            ...
            if self.config.mpi_command:
                ...
                while i < len(mpi_parts):
                    ...
                    if part.startswith("-"):
                        ...
                        # Validate the argument value (e.g., the "4" in "-n 4" or "hosts.txt" in "-hostfile hosts.txt")
                        if i + 1 >= len(mpi_parts):
                            msg = f"Missing value for MPI flag: {part}"
                            raise ValueError(msg)

                        val = mpi_parts[i+1]
                        if part in {"-n", "-np", "-N", "--n"}:
                            if not val.isdigit():
                                msg = f"MPI rank argument must be numeric, got: {val}"
                                raise ValueError(msg)
                        elif part in {"-hostfile", "-machinefile"}:
                            import re
                            if not re.match(r"^[a-zA-Z0-9_\-\.\/]+$", val):
                                msg = f"Forbidden characters in MPI hostfile path: {val}"
                                raise ValueError(msg)

                        i += 2  # skip the flag and its value (e.g., -n 4)
                    else:
                        # Extra bare values or unknown arguments are not allowed
