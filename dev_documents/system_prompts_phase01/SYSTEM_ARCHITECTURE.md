...
**Cycle 01 Features & API Specifications:**
*   **Pydantic Workflow Models (`domain_models/workflow.py`):** Meticulously implement the foundational `ActiveLearningThresholds` and `CutoutConfig` strict Pydantic models. Ensure rigorous cross-field validation rules are established using `@model_validator` (e.g., verifying that `core_radius` is strictly less than `buffer_radius`).
*   **Intelligent Cutout Logic (`utils/extraction.py`):** Comprehensively refactor the existing extraction module to introduce the highly advanced `extract_intelligent_cluster(structure: Atoms, target_atoms: List[int], config: CutoutConfig) -> Atoms` function. This function must perfectly execute complex spatial partitioning based on the provided radii parameters without mutating the original massive structure.
*   **Two-Tier Evaluator (`utils/extraction.py`):** Implement the sophisticated Two-Tier Evaluator mathematical logic. Define `identify_epicentre(forces: np.ndarray, uncertainties: np.ndarray, thresholds: ActiveLearningThresholds) -> List[int]` to precisely identify the specific epicentre atoms based on the strict `threshold_add_train` parameter, distinctly separating them from the broader `threshold_call_dft` trigger zone.
*   **Auto-Passivation Algorithm (`utils/structure.py`):** Develop the critical, complex auto-passivation logic. Create `passivate_dangling_bonds(cluster: Atoms, target_element: str = "H") -> Atoms`. This must involve analyzing bond lengths and standard electronegativity tables to intelligently add appropriate dummy atoms (like fractional hydrogen) to severed dangling bonds, ensuring absolute charge neutrality and preventing dipole divergence.
*   **Abstract Interface Definition (`core/base.py`):** Define the rigorous `BaseOracle` abstract interface early to prevent circular dependencies in testing. Create a robust, deterministic mock `MACEManager` class conforming to `BaseOracle`. This mock is absolutely critical to support the rigorous testing of the pre-relaxation algorithms without requiring the presence of massive, GPU-dependent external foundation models during standard unit test execution.
...
**Cycle 02 Features & API Specifications:**
*   **LammpsEngine Modernization (`core/engine.py`):** Significantly update the `LammpsEngine` class to natively support seamless resume capabilities. Introduce the `run(self, resume_from_step: int = 0, override_n_steps: int = None)` signature. This requires deep structural changes to how the Python process manages the subprocess lifecycle.
*   **Master-Slave Integration:** Implement the definitive Master-Slave inversion. This must be achieved either by utilizing the highly advanced LAMMPS `fix python/invoke` functionality to call Python callbacks directly from C++, or by developing an overwhelmingly robust `.restart` file management system that flawlessly preserves the exact, precise MD state (all atomic coordinates, all velocities, and the exact simulation timestep) across consecutive halts and restarts.
*   **Soft-Start Thermodynamics:** Develop critical soft-start thermodynamic logic (e.g., the temporary application of a highly aggressive Langevin thermostat for the first 10-50 steps following a resume). This is absolutely necessary to prevent disastrous, system-destroying energy spikes that inevitably occur upon resuming a simulation with a newly updated, slightly shifted mathematical potential.
*   **Decoupled Output Parsing (`utils/lammps_parser.py`):** Strictly decouple all complex LAMMPS output parsing logic (reading thermodynamics, extracting stresses) out of the execution engine and into a newly dedicated, highly testable `LammpsResultParser` class, adhering strictly to the Single Responsibility Principle.
*   **Resource Management:** Ensure that any temporary files or `.restart` directories generated during this complex handoff process are rigorously tracked and securely cleaned up using proper context managers (e.g., `tempfile.TemporaryDirectory`) to prevent uncontrolled disk space exhaustion on HPC nodes.
...
**Cycle 03 Features & API Specifications:**
*   **Foundation Model Oracle (`core/oracle.py`):** Implement the comprehensive, fully functional `MACEManager(BaseOracle)` class, heavily leveraging the external MACE foundation model. This integration must be highly robust, properly handling GPU memory allocation, and must implement the required `predict(self, structures: List[Atoms]) -> Dict` method capable of extracting not just energies and forces, but the critical mathematical uncertainty outputs required for active learning.
*   **Intelligent Routing (`core/oracle.py`):** Develop the highly intelligent `TieredOracle(BaseOracle)` wrapper class. This component must contain complex, deterministic routing logic designed to seamlessly route evaluation requests between the fast MACE model and the slow `DFTManager` based exclusively on the rigorously evaluated uncertainty metrics.
*   **Phase 1 Distillation Logic (`core/generator.py`):** Implement the complete Phase 1 Zero-Shot Distillation mathematical logic. This encompasses building a `CombinatorialGenerator` to create massive structure pools, integrating the legacy `ActiveSetSelector` (DIRECT sampling) to reduce this pool to an information-dense core, and applying strict MACE confidence filtering via `MACEManager`.
*   **Baseline Delta Learning Config (`domain_models/config.py`):** Integrate baseline Delta Learning configurations. Ensure the `PacemakerTrainer` configuration can automatically generate the correct parameters to train against a safe, physics-based repulsive baseline (like Lennard-Jones or ZBL potentials) to physically prevent atoms from catastrophically overlapping in highly compressed states.
*   **Robust Fallback Mechanisms:** Establish strict fallback mechanisms; if the MACE model fails to load or infer correctly due to network or GPU errors, the system must gracefully handle the failure via exponential backoff retries or appropriately fallback to safe defaults, alerting the orchestrator rather than silently crashing.
...
**Cycle 04 Features & API Specifications:**
*   **Incremental Trainer API (`core/trainer.py`):** Substantially update the `PacemakerTrainer` class to fully support complex incremental updates. Introduce `incremental_train(self, new_data: List[Atoms], replay_buffer: List[Atoms])`. This requires modifying how data is fed into the underlying Pacemaker executable, moving away from monolithic `.extxyz` data files to targeted update sets.
*   **Replay Buffer Algorithm (`core/trainer.py`):** Implement highly sophisticated replay buffer mathematical logic via `get_replay_buffer(self, max_size: int) -> List[Atoms]`. This system must intelligently mix a statistically sound sample of historical data with the newly acquired Active Learning data, utilizing specific weighting algorithms to definitively prevent catastrophic forgetting of previously learned stable bulk structures.
*   **Awakening MACE (`core/trainer.py`):** Develop the specialized `FinetuneManager`. This critical component must implement `finetune_model(self, model: Any, ground_truth: List[Atoms])`. It is designed to briefly and highly efficiently tune specifically the readout layers of the MACE PyTorch model based exclusively on the newly acquired, pristine DFT data, creating an "awakened" model locally specialized for the current anomaly.
*   **Surrogate Data Generation (`core/generator.py`):** Implement explosive surrogate data generation utilizing this newly awakened MACE model. The system must rapidly generate thousands of synthetic data points (`SurrogateGenerator.generate()`) by exploring the phase space immediately surrounding the halted epicentre, allowing the subsequent ACE delta learning to train on a massive, highly relevant dataset safely generated without additional DFT costs.
*   **Memory Optimization:** Ensure that all dataset manipulations, concatenations, and sub-sampling routines are meticulously optimized using efficient NumPy/ASE array operations (e.g., `atoms.arrays.get('positions')`) to strictly maintain O(1) memory profiling, preventing memory exhaustion when dealing with million-atom systems.
...
**Cycle 05 Features & API Specifications:**
*   **Automated Validation (`core/validator.py`):** Significantly enhance the `Validator` class to fully automate the critical Phase 2 physical checks. This involves programmatically executing Phonon dispersion calculations, Elastic constant derivations, and high-temperature Mini-MD stress tests, complete with automated retry loops extending Phase 1 sampling if stability criteria are not met.
*   **Resilient State Repository (`core/state.py`):** Implement an overwhelmingly robust SQLite or highly structured JSON-based task-level checkpointing system via a `StateRepository` class. This must save the exact system state (`LoopState`) not just at the end of an iteration, but continuously during long surrogate generation loops or after every single DFT calculation, ensuring rapid recovery after brutal HPC wall-time terminations.
*   **Artifact Cleanup Daemons (`utils/io.py`):** Develop relentless, parallel artifact cleanup daemon processes. These critical background tasks must actively compress or permanently remove massive `.wfc` (wavefunction) and LAMMPS dump files immediately after they are successfully consumed by the learning or inference processes, categorically preventing storage quota exhaustion.
*   **Orchestrator State Machine (`core/orchestrator.py`):** Finalize the primary orchestrator loop, meticulously and seamlessly linking Phase 1 (Distillation) through Phase 4 (Fine-tuning and Resume), ensuring that all strict architectural boundaries, error handling protocols, and immutability constraints are flawlessly respected throughout the entire lifecycle.
*   **End-to-End Validation:** Conduct comprehensive, holistic End-to-End (E2E) testing encompassing all five cycles simultaneously, simulating massive system crashes, thermal noise spikes, and complex phase transitions to categorically prove the system's absolute resilience and scientific validity.
